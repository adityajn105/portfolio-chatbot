...
url: https://adityajain.me/blogs/policy-optimization-rl.html
---
...
State: A set of tokens that represent every condition that agent can be in. Model: model or transition model gives an action’s effect in state. In particular T(s,a,s′)T(s,a,s')T(s,a,s′) defines transition T where being in state S, taking action ‘a’ takes us to state ‘s’. For stochastic actions (noisy, non-deterministic) we define probabiliity P(S′∣S,a)P(S'|S,a)P(S′∣S,a) which represent probability of reaching S’ if action ‘a’ is taken in state S. Action: Action ‘a’ is set of all possible decision, a(s) defines set of action that can be taken in state S. Reward: It is a real-valued response to action. R(s)R(s)R(s) indicate reward for being in state ‘s’. R(s,a)R(s,a)R(s,a) is reward for being in state ‘s’ and taking action ‘a’. R(s,a,s′)R(s,a,s')R(s,a,s′) indicates reward for being in state ‘s’, taking action ‘a’ and ending up in state ‘s’. Policy: It is a solution to Markov Decision Process. It is set of actions that are taken by agent to reach goal. It indicates action ‘a’ to be taken which in state S. A policy is denoted as ‘Pi’ as pi(s)pi(s)pi(s) -> ∞\infty∞
...
- SSS: Set of Observations

- AAA: Set of Actions

- TTT: Transition Model

- rrr: Reward Model

- γ\gammaγ: Discount factor (between 0 & 1) represent relative importance between immediate and future reward.
...
V(s)=maxa(R(s,a)+γV(s′))V(s) = max_a \Bigl( R(s,a) + \gamma V(s') \Bigl)V(s)=maxa​(R(s,a)+γV(s′))

- VVV: Value of given state s

- maxamax_amaxa​: Maximum for action a

- RRR: reward for action a in state s

- γ\gammaγ: discount factor (Gamma)

- sss: Next state by choosing action a
...
V(s)=maxa(R(s,a)+γ∑s′P(s,a,s′)V(s′))V(s) = max_a \Bigl( R(s,a) + \gamma \sum_{s'} P(s,a,s') V(s') \Bigl)V(s)=maxa​(R(s,a)+γ∑s′​P(s,a,s′)V(s′))
...
- s′s's′: Next state by choosing action a

### Value Functions

It estimates “how good” a state is for an agent to be in. Equal to expected discounted reward per agent when starting from state ‘s’ and successfully following policy pipipi for an action. May also be referred as ‘Value of policy.’

Two types of value functions.

- State-Value Functions: Expected/Discounted reward when starting in state ‘s’ and successfully following policy 'pipipi' for an action. Denoted as V(s)orVπ(s)V(s) or V_\pi(s)V(s)orVπ​(s). How good is a state.

- Action-Value Function: Action ‘a’ to state ‘s’ and return a real value. Referred as Q-function. Denoted as Q(s,a)Q(s,a)Q(s,a). How good is a state action pair for agent in environment.

Optimal value function is a value function of a state for an optimal policy π∗\pi^\astπ∗, which maximizes discounted reward. Among all value function, there exist a one higher value function for all states denoted by V∗(s)V^\ast(s)V∗(s).
...
Problem: Given a policy pipipi, Find the optimal policy π∗\pi^\astπ∗.

Solution:

Evaluate the policy pipipi.

Vπ(s)=E[Rt+1+γRt+2+....∣st=s]V_{\pi}(s) = E\Big[ R_{t+1} + \gamma R_{t+2} + .... | s_t = s \Big]Vπ​(s)=E[Rt+1​+γRt+2​+....∣st​=s]

Improve the policy by acting greedily with respect to VπV_\piVπ​.

π∗=greedy(Vπ)\pi^\ast = greedy\Big( V_{\pi} \Big)π∗=greedy(Vπ​)

- Policy evaluation - Estimate VpiV_{pi}Vpi​, Iterative policy evaluation

- Policy improvement - Generate π′≥π\pi' \geq \piπ′≥π, Greedy policy improvement

Lets discuss the Policy Evaluation

- Iterative application of Bellman expectation backup.

- V1−>V2−>V3−>....−>VpiV_1 -> V_2 -> V_3 -> .... -> V_{pi}V1​−>V2​−>V3​−>....−>Vpi​. Start with random value function and iteratively figure out new value function.

- Using Synchronous backups:

at each iteration k+1

For all state s\inSs\inSs\inS

Update Vk+1(s)V_{k+1}(s)Vk+1​(s) from Vk(s′)V_k(s')Vk​(s′) using bellman expectation equation.

Vk+1(s)=∑a∈Aπ(a∣s)(Rsa+γ∑s′∈SPss′aVk(s′))V_{k+1}(s) = \sum_{a \in A} \pi(a|s)\Big( R_s^a + \gamma \sum_{s' \in S} P_{ss'}^a V_k(s') \Big)Vk+1​(s)=∑a∈A​π(a∣s)(Rsa​+γ∑s′∈S​Pss′a​Vk​(s′))

Vk+1=Rπ+γPπVkV^{k+1} = R^{\pi} + \gamma P^{\pi}V^{k}Vk+1=Rπ+γPπVk

Where s′s's′ is successor of state sss.
...
- This process of policy iteration always converges to π∗\pi^\astπ∗.
...
Problem: Find Optimal Policy π∗\pi^\astπ∗.

Solution: Iterative application of Bellman optimality backup.

- V1−>V2−>V3−>....−>VastV_1 -> V_2 -> V_3 -> .... -> V_astV1​−>V2​−>V3​−>....−>Va​st. Start with random value function and update values using bellman optimality equation.
...
Update Vk+1(s)V_{k+1}(s)Vk+1​(s) from Vk(s′)V_k(s')Vk​(s′) using bellman optimality equation.

Vk+1(s)=max⁡a∈A(Rsa+γ∑s′∈SPss′aVk(s′))V_{k+1}(s) = \max_{a \in A} \Big( R^a_s + \gamma \sum_{s' \in S} P^a_{ss'} V_k(s') \Big)Vk+1​(s)=maxa∈A​(Rsa​+γ∑s′∈S​Pss′a​Vk​(s′))

Vk+1=max⁡a∈A(Ra+γPaVk)V_{k+1} = \max_{a \in A} \Big( R^a + \gamma P^aV_k \Big)Vk+1​=maxa∈A​(Ra+γPaVk​)
...
- Algorithms are based on state-value function Vπ(s)orVast(s)V_\pi(s) or V_ast(s)Vπ​(s)orVa​st(s)

- Complexity O(mn2)O(mn^2 )O(mn2) per iteration, for m actions and n states

- Could also apply to action-value function qpi(s,a)orqast(s,a)q_pi(s, a) or q^ast(s, a)qp​i(s,a)orqast(s,a)

- Complexity O(m2n2)O(m^2n^2 )O(m2n2) per iteration
