...
url: https://adityajain.me/blogs/deep-q-learning.html
---
...
- In the enviroments where there is no discrete states and there are continuous values represnting state of environment. Then it is not possible to use traditional Q-Learning.
...
state-action-valuet=rewardt+γ∗maxa(state-action-valuet+1)\text{state-action-value}_t = reward_t + \gamma * max_a (\text{state-action-value}_{t+1})state-action-valuet​=rewardt​+γ∗maxa​(state-action-valuet+1​)
...
Q(s,a)=V(s)+(A(s,a)−meana∈A(A(s,a)))Q(s,a) = V(s) + \Big( A(s,a) - mean_{a \in A}(A(s,a)) \Big)Q(s,a)=V(s)+(A(s,a)−meana∈A​(A(s,a)))
...
P(i)=pia∑kpkaP(i) = \frac{p^a_i}{ \sum_k p^a_k}P(i)=∑k​pka​pia​​
...
- Prioritized Experience Replay by Google Deep Mind
